...
public:
    ...
    static bool HandleGMVisibleCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
        {
            handler->PSendSysMessage(LANG_YOU_ARE, handler->GetSession()->GetPlayer()->isGMVisible() ? handler->GetORIGINString(LANG_VISIBLE) : handler->GetORIGINString(LANG_INVISIBLE));
            return true;
        }

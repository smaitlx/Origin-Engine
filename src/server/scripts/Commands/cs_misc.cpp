...
public:
    ...
    static bool HandleGPSCommand(ChatHandler* handler, char const* args)
    {
        ...
        CellCoord cellCoord = ORIGIN::ComputeCellCoord(object->GetPositionX(), object->GetPositionY());
        Cell cell(cellCoord);
        ...
        GridCoord gridCoord = ORIGIN::ComputeGridCoord(object->GetPositionX(), object->GetPositionY());
    ...
    static bool HandleSummonCommand(ChatHandler* handler, char const* args)
    {
        ...
        else
        {
            ...
            handler->PSendSysMessage(LANG_SUMMONING, nameLink.c_str(), handler->GetORIGINString(LANG_OFFLINE));
    ...
    static bool HandleCooldownCommand(ChatHandler* handler, char const* args)
    {
        ...
        else
        {
            ...
            {
                handler->PSendSysMessage(LANG_UNKNOWN_SPELL, target == handler->GetSession()->GetPlayer() ? handler->GetORIGINString(LANG_YOU) : nameLink.c_str());
                handler->SetSentErrorMessage(true);
                return false;
            }

            target->RemoveSpellCooldown(spellIid, true);
            handler->PSendSysMessage(LANG_REMOVE_COOLDOWN, spellIid, target == handler->GetSession()->GetPlayer() ? handler->GetORIGINString(LANG_YOU) : nameLink.c_str());
        }
        return true;
    }
    ...
    static bool HandleNearGraveCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (graveyard)
        {
            ...
            std::string team_name = handler->GetORIGINString(LANG_COMMAND_GRAVEYARD_NOTEAM);

            if (team == 0)
                team_name = handler->GetORIGINString(LANG_COMMAND_GRAVEYARD_ANY);
            else if (team == HORDE)
                team_name = handler->GetORIGINString(LANG_COMMAND_GRAVEYARD_HORDE);
            else if (team == ALLIANCE)
                team_name = handler->GetORIGINString(LANG_COMMAND_GRAVEYARD_ALLIANCE);
    ...
    static bool HandleAddItemCommand(ChatHandler* handler, char const* args)
    {
        ...
        TC_LOG_DEBUG(LOG_FILTER_GENERAL, handler->GetORIGINString(LANG_ADDITEM), itemId, count);
    ...
    static bool HandleAddItemSetCommand(ChatHandler* handler, char const* args)
    {
        ...
        TC_LOG_DEBUG(LOG_FILTER_GENERAL, handler->GetORIGINString(LANG_ADDITEMSET), itemSetId);
    ...
    **/
    ...
    static bool HandlePInfoCommand(ChatHandler* handler, char const* args)
    {
        ...
        std::string userName          = handler->GetORIGINString(LANG_ERROR);
        uint32 accId                  = 0;
        uint32 lowguid                = GUID_LOPART(targetGuid);
        std::string eMail             = handler->GetORIGINString(LANG_ERROR);
        std::string regMail           = handler->GetORIGINString(LANG_ERROR);
        uint32 security               = 0;
        std::string lastIp            = handler->GetORIGINString(LANG_ERROR);
        uint8 locked                  = 0;
        std::string lastLogin         = handler->GetORIGINString(LANG_ERROR);
        ...
        std::string genderStr           = handler->GetORIGINString(LANG_ERROR);
        uint32 totalPlayerTime          = 0;
        uint8 level                     = 0;
        std::string alive               = handler->GetORIGINString(LANG_ERROR);
        uint32 money                    = 0;
        uint32 xp                       = 0;
        uint32 xptotal                  = 0;
        ...
        handler->PSendSysMessage(LANG_PINFO_PLAYER, target ? "" : handler->GetORIGINString(LANG_OFFLINE), nameLink.c_str(), lowguid);
        ...
        handler->PSendSysMessage(LANG_PINFO_CHR_RACE, (gender == 0 ? handler->GetORIGINString(LANG_CHARACTER_GENDER_MALE) : handler->GetORIGINString(LANG_CHARACTER_GENDER_FEMALE)), raceStr.c_str(), classStr.c_str());
    ...
    static bool HandleRespawnCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        CellCoord p(ORIGIN::ComputeCellCoord(player->GetPositionX(), player->GetPositionY()));
        Cell cell(p);
        cell.SetNoCreate();

        ORIGIN::RespawnDo u_do;
        ORIGIN::WorldObjectWorker<ORIGIN::RespawnDo> worker(player, u_do);

        TypeContainerVisitor<ORIGIN::WorldObjectWorker<ORIGIN::RespawnDo>, GridTypeMapContainer > obj_worker(worker);

...
public:
    ...
    static bool HandleModifyEnergyCommand(ChatHandler* handler, const char* args)
    {
        ...
        TC_LOG_DEBUG(LOG_FILTER_GENERAL, handler->GetORIGINString(LANG_CURRENT_ENERGY), target->GetMaxPower(POWER_ENERGY));
    ...
    static bool HandleModifyMoneyCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (moneyToAdd < 0)
        {
            ...
            TC_LOG_DEBUG(LOG_FILTER_GENERAL, handler->GetORIGINString(LANG_CURRENT_MONEY), targetMoney, moneyToAdd, newmoney);
            if (newmoney <= 0)
            {
        ...
        TC_LOG_DEBUG(LOG_FILTER_GENERAL, handler->GetORIGINString(LANG_NEW_MONEY), targetMoney, moneyToAdd, target->GetMoney());
    ...
    static bool HandleModifyRepCommand(ChatHandler* handler, const char* args)
    {
        ...
        if ((amount == 0) && (rankTxt[0] != '-') && !isdigit(rankTxt[0]))
        {
            ...
            {
                std::string rank = handler->GetORIGINString(ReputationRankStrIndex[r]);
                if (rank.empty())
                    continue;

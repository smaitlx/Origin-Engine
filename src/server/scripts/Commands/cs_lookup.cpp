    ...
    static bool HandleLookupEventCommand(ChatHandler* handler, char const* args)
    {
        ...
        for (uint32 id = 0; id < events.size(); ++id)
        {
            ...
            if (Utf8FitTo(descr, wNamePart))
            {
                ...
                char const* active = activeEvents.find(id) != activeEvents.end() ? handler->GetORIGINString(LANG_ACTIVE) : "";
            ...
            if (factionEntry)
            {
                ...
                if (locale < TOTAL_LOCALES)
                {
                    ...
                    if (factionState) // and then target != NULL also
                    {
                        ...
                        std::string rankName = handler->GetORIGINString(index);

                        ss << ' ' << rankName << "|h|r (" << target->GetReputationMgr().GetReputation(factionEntry) << ')';

                        if (factionState->Flags & FACTION_FLAG_VISIBLE)
                            ss << handler->GetORIGINString(LANG_FACTION_VISIBLE);
                        if (factionState->Flags & FACTION_FLAG_AT_WAR)
                            ss << handler->GetORIGINString(LANG_FACTION_ATWAR);
                        if (factionState->Flags & FACTION_FLAG_PEACE_FORCED)
                            ss << handler->GetORIGINString(LANG_FACTION_PEACE_FORCED);
                        if (factionState->Flags & FACTION_FLAG_HIDDEN)
                            ss << handler->GetORIGINString(LANG_FACTION_HIDDEN);
                        if (factionState->Flags & FACTION_FLAG_INVISIBLE_FORCED)
                            ss << handler->GetORIGINString(LANG_FACTION_INVISIBLE_FORCED);
                        if (factionState->Flags & FACTION_FLAG_INACTIVE)
                            ss << handler->GetORIGINString(LANG_FACTION_INACTIVE);
                    }
                    else
                        ss << handler->GetORIGINString(LANG_FACTION_NOREPUTATION);
                            ...
                            if (target)
                            {
                                ...
                                switch (status)
                                {
                                    case QUEST_STATUS_COMPLETE:
                                        statusStr = handler->GetORIGINString(LANG_COMMAND_QUEST_COMPLETE);
                                        break;
                                    case QUEST_STATUS_INCOMPLETE:
                                        statusStr = handler->GetORIGINString(LANG_COMMAND_QUEST_ACTIVE);
                                        break;
                                    case QUEST_STATUS_REWARDED:
                                        statusStr = handler->GetORIGINString(LANG_COMMAND_QUEST_REWARDED);
                                        break;
                                    default:
                                        break;
                                }
        ...
        for (uint32 id = 0; id < sSkillLineStore.GetNumRows(); id++)
        {
            ...
            if (skillInfo)
            {
                ...
                if (locale < TOTAL_LOCALES)
                {
                    ...
                    {
                        knownStr = handler->GetORIGINString(LANG_KNOWN);
                        uint32 curValue = target->GetPureSkillValue(id);
                        uint32 maxValue  = target->GetPureMaxSkillValue(id);
                        uint32 permValue = target->GetSkillPermBonusValue(id);
                        uint32 tempValue = target->GetSkillTempBonusValue(id);

                        char const* valFormat = handler->GetORIGINString(LANG_SKILL_VALUES);
        ...
        for (uint32 id = 0; id < sSpellMgr->GetSpellInfoStoreSize(); id++)
        {
            ...
            if (spellInfo)
            {
                ...
                if (locale < TOTAL_LOCALES)
                {
                    ...
                    if (rank)
                        ss << handler->GetORIGINString(LANG_SPELL_RANK) << rank;
                    ...
                    if (talent)
                        ss << handler->GetORIGINString(LANG_TALENT);
                    if (passive)
                        ss << handler->GetORIGINString(LANG_PASSIVE);
                    if (learn)
                        ss << handler->GetORIGINString(LANG_LEARN);
                    if (known)
                        ss << handler->GetORIGINString(LANG_KNOWN);
                    if (active)
                        ss << handler->GetORIGINString(LANG_ACTIVE);
        ...
        for (uint32 id = 0; id < sCharTitlesStore.GetNumRows(); id++)
        {
            ...
            if (titleInfo)
            {
                ...
                if (locale < TOTAL_LOCALES)
                {
                    ...
                    char const* knownStr = target && target->HasTitle(titleInfo) ? handler->GetORIGINString(LANG_KNOWN) : "";

                    char const* activeStr = target && target->GetUInt32Value(PLAYER_CHOSEN_TITLE) == titleInfo->bit_index
                        ? handler->GetORIGINString(LANG_ACTIVE)
                        : "";
            ...
            if (MapEntry const* mapInfo = sMapStore.LookupEntry(id))
            {
                ...
                if (Utf8FitTo(name, wNamePart) && locale < TOTAL_LOCALES)
                {
                    ...
                    if (mapInfo->IsContinent())
                        ss << handler->GetORIGINString(LANG_CONTINENT);

                    switch (mapInfo->map_type)
                    {
                        case MAP_INSTANCE:
                            ss << handler->GetORIGINString(LANG_INSTANCE);
                            break;
                        case MAP_RAID:
                            ss << handler->GetORIGINString(LANG_RAID);
                            break;
                        case MAP_BATTLEGROUND:
                            ss << handler->GetORIGINString(LANG_BATTLEGROUND);
                            break;
                        case MAP_ARENA:
                            ss << handler->GetORIGINString(LANG_ARENA);
                            break;
                    }

...
public:
    ...
    static bool HandleAnnounceCommand(ChatHandler* handler, char const* args)
    {
        ...
        char buff[2048];
        sprintf(buff, handler->GetORIGINString(LANG_SYSTEMMESSAGE), args);
    ...
    static bool HandleNotifyCommand(ChatHandler* handler, char const* args)
    {
        ...
        std::string str = handler->GetORIGINString(LANG_GLOBAL_NOTIFY);
        str += args;
    ...
    static bool HandleGMNotifyCommand(ChatHandler* handler, char const* args)
    {
        ...
        std::string str = handler->GetORIGINString(LANG_GM_NOTIFY);
        str += args;
    ...
    static bool HandleWhispersCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
        {
            handler->PSendSysMessage(LANG_COMMAND_WHISPERACCEPTING, handler->GetSession()->GetPlayer()->isAcceptWhispers() ?  handler->GetORIGINString(LANG_ON) : handler->GetORIGINString(LANG_OFF));
            return true;
        }

...
/// @todo clean up this workaround when ORIGIN adds support to do it properly (with gossip selections instead of instant summon)
    ...
    struct npc_phase_hunterAI : public ScriptedAI
    {
        ...
        void UpdateAI(uint32 diff) OVERRIDE
        {
            ...
            if (ManaBurnTimer <= diff)
            {
                ...
                {
                    DoCast(ORIGIN::Containers::SelectRandomContainerElement(UnitsWithMana), SPELL_MANA_BURN);

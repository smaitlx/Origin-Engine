...
void hyjalAI::HideNearPos(float x, float y)
{
    ...
    ORIGIN::AllFriendlyCreaturesInGrid creature_check(me);
    ORIGIN::CreatureListSearcher<ORIGIN::AllFriendlyCreaturesInGrid> creature_searcher(me, creatures, creature_check);

    TypeContainerVisitor <ORIGIN::CreatureListSearcher<ORIGIN::AllFriendlyCreaturesInGrid>, GridTypeMapContainer> creature_visitor(creature_searcher);
...
void hyjalAI::RespawnNearPos(float x, float y)
{
    ...
    ORIGIN::WorldObjectWorker<ORIGIN::RespawnDo> worker(me, u_do);
...
void hyjalAI::WaypointReached(uint32 waypointId)
{
    if (waypointId == 1 || (waypointId == 0 && me->GetEntry() == THRALL))
    {
        ...
        TypeContainerVisitor
            <ORIGIN::CreatureListSearcher<ORIGIN::AllFriendlyCreaturesInGrid>,

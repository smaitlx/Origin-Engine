                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_BELLOWING_ROAR: // Phase PHASE_END
                        {
                            ...
                            ORIGIN::GameObjectInRangeCheck check(me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 15);
                            ORIGIN::GameObjectLastSearcher<ORIGIN::GameObjectInRangeCheck> searcher(me, Floor, check);

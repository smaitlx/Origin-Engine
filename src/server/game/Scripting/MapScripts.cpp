...
inline GameObject* Map::_FindGameObject(WorldObject* searchObject, uint32 guid) const
{
    ...
    CellCoord p(ORIGIN::ComputeCellCoord(searchObject->GetPositionX(), searchObject->GetPositionY()));
    Cell cell(p);

    ORIGIN::GameObjectWithDbGUIDCheck goCheck(*searchObject, guid);
    ORIGIN::GameObjectSearcher<ORIGIN::GameObjectWithDbGUIDCheck> checker(searchObject, gameobject, goCheck);

    TypeContainerVisitor<ORIGIN::GameObjectSearcher<ORIGIN::GameObjectWithDbGUIDCheck>, GridTypeMapContainer > objectChecker(checker);
        ...
        switch (step.script->command)
        {
            case SCRIPT_COMMAND_TALK:
                ...
                if (step.script->Talk.Flags & SF_TALK_USE_PLAYER)
                {
                    if (Player* player = _GetScriptPlayerSourceOrTarget(source, target, step.script))
                    {
                        ...
                        std::string text(sObjectMgr->GetORIGINString(step.script->Talk.TextID, loc_idx));
            ...
            case SCRIPT_COMMAND_CALLSCRIPT_TO_UNIT:
            {
                ...
                if (source) //using grid searcher
                {
                    ...
                    CellCoord p(ORIGIN::ComputeCellCoord(wSource->GetPositionX(), wSource->GetPositionY()));
                    Cell cell(p);

                    ORIGIN::CreatureWithDbGUIDCheck target_check(wSource, step.script->CallScript.CreatureEntry);
                    ORIGIN::CreatureSearcher<ORIGIN::CreatureWithDbGUIDCheck> checker(wSource, cTarget, target_check);

                    TypeContainerVisitor<ORIGIN::CreatureSearcher <ORIGIN::CreatureWithDbGUIDCheck>, GridTypeMapContainer > unit_checker(checker);

...
void Map::SwitchGridContainers(Creature* obj, bool on)
{
    ...
    CellCoord p = ORIGIN::ComputeCellCoord(obj->GetPositionX(), obj->GetPositionY());
    if (!p.IsCoordValid())
    {
...
{
    ORIGIN_GUARD(ACE_Thread_Mutex, GridLock);
...
{
    CellCoord cellCoord = ORIGIN::ComputeCellCoord(player->GetPositionX(), player->GetPositionY());
...
bool Map::AddToMap(T *obj)
{
    ...
    CellCoord cellCoord = ORIGIN::ComputeCellCoord(obj->GetPositionX(), obj->GetPositionY());
...
void Map::VisitNearbyCellsOf(WorldObject* obj, TypeContainerVisitor<ORIGIN::ObjectUpdater, GridTypeMapContainer> &gridVisitor, TypeContainerVisitor<ORIGIN::ObjectUpdater, WorldTypeMapContainer> &worldVisitor)
{
...
void Map::Update(const uint32 t_diff)
{
    ...
    ORIGIN::ObjectUpdater updater(t_diff);
    // for creature
    TypeContainerVisitor<ORIGIN::ObjectUpdater, GridTypeMapContainer  > grid_object_update(updater);
    // for pets
    TypeContainerVisitor<ORIGIN::ObjectUpdater, WorldTypeMapContainer > world_object_update(updater);
    ...
    for (GridRefManager<NGridType>::iterator i = GridRefManager<NGridType>::begin(); i != GridRefManager<NGridType>::end(); ++i)
    {
        ...
        for (uint32 x = cell_min.x_coord; x < cell_max.x_coord; ++x)
        {
            for (uint32 y = cell_min.y_coord; y < cell_max.y_coord; ++y)
            {
                ...
                ORIGIN::DelayedUnitRelocation cell_relocation(cell, pair, *this, MAX_VISIBILITY_DISTANCE);
                TypeContainerVisitor<ORIGIN::DelayedUnitRelocation, GridTypeMapContainer  > grid_object_relocation(cell_relocation);
                TypeContainerVisitor<ORIGIN::DelayedUnitRelocation, WorldTypeMapContainer > world_object_relocation(cell_relocation);
...
{
    cell.SetNoCreate();
    ORIGIN::VisibleChangesNotifier notifier(*obj);
    TypeContainerVisitor<ORIGIN::VisibleChangesNotifier, WorldTypeMapContainer > player_notifier(notifier);
...
{
    ORIGIN::VisibleNotifier notifier(*player);

    cell.SetNoCreate();
    TypeContainerVisitor<ORIGIN::VisibleNotifier, WorldTypeMapContainer > world_notifier(notifier);
    TypeContainerVisitor<ORIGIN::VisibleNotifier, GridTypeMapContainer  > grid_notifier(notifier);
...
void Map::AddToActive(Creature* c)
{
    ...
    if (!c->IsPet() && c->GetDBTableGUIDLow())
    {
        ...
        GridCoord p = ORIGIN::ComputeGridCoord(x, y);
        if (getNGrid(p.x_coord, p.y_coord))
            getNGrid(p.x_coord, p.y_coord)->incUnloadActiveLock();
        else
        {
            GridCoord p2 = ORIGIN::ComputeGridCoord(c->GetPositionX(), c->GetPositionY());
...
bool InstanceMap::AddPlayerToMap(Player* player)
{
    ...
    {
        ORIGIN_GUARD(ACE_Thread_Mutex, Lock);

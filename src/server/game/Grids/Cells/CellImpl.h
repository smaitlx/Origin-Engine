...
#ifndef ORIGIN_CELLIMPL_H
#define ORIGIN_CELLIMPL_H
...
inline CellArea Cell::CalculateCellArea(float x, float y, float radius)
{
    if (radius <= 0.0f)
    {
        CellCoord center = ORIGIN::ComputeCellCoord(x, y).normalize();
        return CellArea(center, center);
    }

    CellCoord centerX = ORIGIN::ComputeCellCoord(x - radius, y - radius).normalize();
    CellCoord centerY = ORIGIN::ComputeCellCoord(x + radius, y + radius).normalize();

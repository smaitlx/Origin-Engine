...
void BattlegroundMgr::BuildBattlegroundListPacket(WorldPacket* data, uint64 guid, Player* player, BattlegroundTypeId bgTypeId, uint8 fromWhere)
{
    ...
    winner_kills = ORIGIN::Honor::hk_honor_at_level(player->getLevel(), float(winner_kills));
    loser_kills = ORIGIN::Honor::hk_honor_at_level(player->getLevel(), float(loser_kills));

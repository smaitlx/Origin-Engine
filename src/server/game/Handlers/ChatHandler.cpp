...
void WorldSession::HandleMessagechatOpcode(WorldPacket& recvData)
{
    ...
    else
    {
        ...
        if (!sender->CanSpeak())
        {
            ...
            SendNotification(GetORIGINString(LANG_WAIT_BEFORE_SPEAKING), timeStr.c_str());
    ...
    {
        SendNotification(GetORIGINString(LANG_GM_SILENCE), sender->GetName().c_str());
        recvData.rfinish();
        return;
    }
    ...
    switch (type)
    {
        ...
        case CHAT_MSG_YELL:
        {
            ...
            {
                SendNotification(GetORIGINString(LANG_SAY_REQ), sWorld->getIntConfig(CONFIG_CHAT_SAY_LEVEL_REQ));
                return;
            }
        ...
        case CHAT_MSG_WHISPER:
        {
            ...
            {
                SendNotification(GetORIGINString(LANG_WHISPER_REQ), sWorld->getIntConfig(CONFIG_CHAT_WHISPER_LEVEL_REQ));
                return;
            }
            ...
            {
                SendNotification(GetORIGINString(LANG_GM_SILENCE), GetPlayer()->GetName().c_str());
                return;
            }
        ...
        case CHAT_MSG_CHANNEL:
        {
            if (!HasPermission(RBAC_PERM_SKIP_CHECK_CHAT_CHANNEL_REQ))
            {
                ...
                {
                    SendNotification(GetORIGINString(LANG_CHANNEL_REQ), sWorld->getIntConfig(CONFIG_CHAT_CHANNEL_LEVEL_REQ));
                    return;
                }
            }
        ...
        case CHAT_MSG_AFK:
        {
            if (!_player->IsInCombat())
            {
                ...
                {
                    _player->autoReplyMsg = msg.empty() ? GetORIGINString(LANG_PLAYER_AFK_DEFAULT) : msg;
        ...
        case CHAT_MSG_DND:
        {
            ...
            {
                _player->autoReplyMsg = msg.empty() ? GetORIGINString(LANG_PLAYER_DND_DEFAULT) : msg;
...
void WorldSession::HandleTextEmoteOpcode(WorldPacket& recvData)
{
    ...
    CellCoord p = ORIGIN::ComputeCellCoord(GetPlayer()->GetPositionX(), GetPlayer()->GetPositionY());

    Cell cell(p);
    cell.SetNoCreate();

    ORIGIN::EmoteChatBuilder emote_builder(*GetPlayer(), text_emote, emoteNum, unit);
    ORIGIN::LocalizedPacketDo<ORIGIN::EmoteChatBuilder > emote_do(emote_builder);
    ORIGIN::PlayerDistWorker<ORIGIN::LocalizedPacketDo<ORIGIN::EmoteChatBuilder > > emote_worker(GetPlayer(), sWorld->getFloatConfig(CONFIG_LISTEN_RANGE_TEXTEMOTE), emote_do);
    TypeContainerVisitor<ORIGIN::PlayerDistWorker<ORIGIN::LocalizedPacketDo<ORIGIN::EmoteChatBuilder> >, WorldTypeMapContainer> message(emote_worker);

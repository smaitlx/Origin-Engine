...
{
    char const* format = GetORIGINString(string_id);
    if (format)
    {
        va_list ap;
        char szStr[1024];
...
const char *WorldSession::GetORIGINString(int32 entry) const
{
    return sObjectMgr->GetORIGINString(entry, GetSessionDbLocaleIndex());
}

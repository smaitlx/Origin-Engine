...
{
    // NOT USED BY ANY SPELL OR USELESS OR IMPLEMENTED IN DIFFERENT WAY IN ORIGIN
}
        ...
        case SPELLFAMILY_PALADIN:
            switch (m_spellInfo->Id)
            {
                case 31789:                                 // Righteous Defense (step 1)
                {
                    ...
                    {
                        Unit* attacker = ORIGIN::Containers::SelectRandomContainerElement(attackers);
...
void Spell::EffectEnergize(SpellEffIndex effIndex)
{
    ...
    if (m_spellInfo->Id == 45051)
    {
        ...
        if (!avalibleElixirs.empty())
        {
            ...
            m_caster->CastSpell(unitTarget, ORIGIN::Containers::SelectRandomContainerElement(avalibleElixirs), true, m_CastItem);
        }
    }
}
...
void Spell::EffectAddHonor(SpellEffIndex /*effIndex*/)
{
    ...
    if (damage <= 50)
    {
        uint32 honor_reward = ORIGIN::Honor::hk_honor_at_level(unitTarget->getLevel(), float(damage));

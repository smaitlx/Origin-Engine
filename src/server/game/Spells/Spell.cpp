...
void Spell::SelectImplicitConeTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    ...
    {
        ORIGIN::WorldObjectSpellConeTargetCheck check(coneAngle, radius, m_caster, m_spellInfo, selectionType, condList);
        ORIGIN::WorldObjectListSearcher<ORIGIN::WorldObjectSpellConeTargetCheck> searcher(m_caster, targets, check, containerTypeMask);
        SearchTargets<ORIGIN::WorldObjectListSearcher<ORIGIN::WorldObjectSpellConeTargetCheck> >(searcher, containerTypeMask, m_caster, m_caster, radius);
        ...
        if (!targets.empty())
        {
            ...
            if (uint32 maxTargets = m_spellValue->MaxAffectedTargets)
            {
                ...
                ORIGIN::Containers::RandomResizeList(targets, maxTargets);
            }
...
void Spell::SelectImplicitAreaTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    ...
    if (!unitTargets.empty())
    {
        ...
        if (maxSize && power != -1)
        {
            if (Powers(power) == POWER_HEALTH)
            {
                ...
                {
                    unitTargets.sort(ORIGIN::HealthPctOrderPred());
            ...
            else
            {
                ...
                if (unitTargets.size() > maxSize)
                {
                    unitTargets.sort(ORIGIN::PowerPctOrderPred((Powers)power));
                    unitTargets.resize(maxSize);
                }
            }
        }
        ...
        if (uint32 maxTargets = m_spellValue->MaxAffectedTargets)
        {
            ...
            ORIGIN::Containers::RandomResizeList(unitTargets, maxTargets);
        }
    ...
    if (!gObjTargets.empty())
    {
        if (uint32 maxTargets = m_spellValue->MaxAffectedTargets)
        {
            ...
            ORIGIN::Containers::RandomResizeList(gObjTargets, maxTargets);
        }
...
void Spell::SelectImplicitTrajTargets()
{
    ...
    std::list<WorldObject*> targets;
    ORIGIN::WorldObjectSpellTrajTargetCheck check(dist2d, m_targets.GetSrcPos(), m_caster, m_spellInfo);
    ORIGIN::WorldObjectListSearcher<ORIGIN::WorldObjectSpellTrajTargetCheck> searcher(m_caster, targets, check, GRID_MAP_TYPE_MASK_ALL);
    SearchTargets<ORIGIN::WorldObjectListSearcher<ORIGIN::WorldObjectSpellTrajTargetCheck> > (searcher, GRID_MAP_TYPE_MASK_ALL, m_caster, m_targets.GetSrcPos(), dist2d);
    if (targets.empty())
        return;

    targets.sort(ORIGIN::ObjectDistanceOrderPred(m_caster));
...
void Spell::SearchTargets(SEARCHER& searcher, uint32 containerMask, Unit* referer, Position const* pos, float radius)
{
    ...
    if (searchInGrid || searchInWorld)
    {
        ...
        CellCoord p(ORIGIN::ComputeCellCoord(x, y));
        Cell cell(p);
        cell.SetNoCreate();
...
WorldObject* Spell::SearchNearbyTarget(float range, SpellTargetObjectTypes objectType, SpellTargetCheckTypes selectionType, ConditionList* condList)
{
    ...
    if (!containerTypeMask)
        return NULL;
    ORIGIN::WorldObjectSpellNearbyTargetCheck check(range, m_caster, m_spellInfo, selectionType, condList);
    ORIGIN::WorldObjectLastSearcher<ORIGIN::WorldObjectSpellNearbyTargetCheck> searcher(m_caster, target, check, containerTypeMask);
    SearchTargets<ORIGIN::WorldObjectLastSearcher<ORIGIN::WorldObjectSpellNearbyTargetCheck> > (searcher, containerTypeMask, m_caster, m_caster, range);
    return target;
}
...
void Spell::SearchAreaTargets(std::list<WorldObject*>& targets, float range, Position const* position, Unit* referer, SpellTargetObjectTypes objectType, SpellTargetCheckTypes selectionType, ConditionList* condList)
{
    ...
    if (!containerTypeMask)
        return;
    ORIGIN::WorldObjectSpellAreaTargetCheck check(range, position, m_caster, referer, m_spellInfo, selectionType, condList);
    ORIGIN::WorldObjectListSearcher<ORIGIN::WorldObjectSpellAreaTargetCheck> searcher(m_caster, targets, check, containerTypeMask);
    SearchTargets<ORIGIN::WorldObjectListSearcher<ORIGIN::WorldObjectSpellAreaTargetCheck> > (searcher, containerTypeMask, m_caster, position, range);
}
...
GameObject* Spell::SearchSpellFocus()
{
    ...
    ORIGIN::GameObjectFocusCheck check(m_caster, m_spellInfo->RequiresSpellFocus);
    ORIGIN::GameObjectSearcher<ORIGIN::GameObjectFocusCheck> searcher(m_caster, focus, check);
    SearchTargets<ORIGIN::GameObjectSearcher<ORIGIN::GameObjectFocusCheck> > (searcher, GRID_MAP_TYPE_MASK_GAMEOBJECT, m_caster, m_caster, m_caster->GetVisibilityRange());
    return focus;
}

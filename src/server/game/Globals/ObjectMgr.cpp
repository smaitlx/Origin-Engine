...
void ObjectMgr::AddCreatureToGrid(uint32 guid, CreatureData const* data)
{
    ...
    for (uint8 i = 0; mask != 0; i++, mask >>= 1)
    {
        if (mask & 1)
        {
            CellCoord cellCoord = ORIGIN::ComputeCellCoord(data->posX, data->posY);
    ...
    do
    {
        ...
        switch (tmp.command)
        {
            ...
            case SCRIPT_COMMAND_TELEPORT_TO:
            {
                ...
                if (!ORIGIN::IsValidMapCoord(tmp.TeleportTo.DestX, tmp.TeleportTo.DestY, tmp.TeleportTo.DestZ, tmp.TeleportTo.Orientation))
                {
            ...
            {
                if (!ORIGIN::IsValidMapCoord(tmp.TempSummonCreature.PosX, tmp.TempSummonCreature.PosY, tmp.TempSummonCreature.PosZ, tmp.TempSummonCreature.Orientation))
                {
...
void ObjectMgr::LoadPointsOfInterest()
{
    ...
    do
    {
        ...
        if (!ORIGIN::IsValidMapCoord(POI.x, POI.y))
        {
...
bool ObjectMgr::LoadORIGINStrings(const char* table, int32 min_value, int32 max_value)
{
    uint32 oldMSTime = getMSTime();
    ...
    for (ORIGINStringLocaleContainer::iterator itr = _ORIGINStringLocaleStore.begin(); itr != _ORIGINStringLocaleStore.end();)
    {
        if (itr->first >= start_value && itr->first < end_value)
            _ORIGINStringLocaleStore.erase(itr++);
        else
            ++itr;
    }
    ...
    if (!result)
    {
        if (min_value == MIN_ORIGIN_STRING_ID)              // error only in case internal strings
            TC_LOG_ERROR(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 ORIGIN strings. DB table `%s` is empty. Cannot continue.", table);
        else
    ...
    do
    {
        ...
        ORIGINStringLocale& data = _ORIGINStringLocaleStore[entry];
    ...
    if (min_value == MIN_ORIGIN_STRING_ID)
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u ORIGIN strings from table %s in %u ms", count, table, GetMSTimeDiffToNow(oldMSTime));
    else
...
const char *ObjectMgr::GetORIGINString(int32 entry, LocaleConstant locale_idx) const
{
    if (ORIGINStringLocale const* msl = GetORIGINStringLocale(entry))
    {
    ...
    if (entry > 0)
        TC_LOG_ERROR(LOG_FILTER_SQL, "Entry %i not found in `ORIGIN_string` table.", entry);
    else
        TC_LOG_ERROR(LOG_FILTER_SQL, "ORIGIN string entry %i not found in DB.", entry);
    return "<error>";
}
...
{
    if (entry >= ORIGIN_TRAINER_START_REF)
        return;
    ...
    for (ScriptMapMap::const_iterator itrMM = scripts->begin(); itrMM != scripts->end(); ++itrMM)
    {
        for (ScriptMap::const_iterator itrM = itrMM->second.begin(); itrM != itrMM->second.end(); ++itrM)
        {
            switch (itrM->second.command)
            {
                case SCRIPT_COMMAND_TALK:
                {
                    if (!GetORIGINStringLocale (itrM->second.Talk.TextID))
...
{
    LoadORIGINStrings("db_script_string", MIN_DB_SCRIPT_STRING_ID, MAX_DB_SCRIPT_STRING_ID);

    std::set<int32> ids;

    for (int32 i = MIN_DB_SCRIPT_STRING_ID; i < MAX_DB_SCRIPT_STRING_ID; ++i)
        if (GetORIGINStringLocale(i))
            ids.insert(i);
...
bool LoadORIGINStrings(const char* table, int32 start_value, int32 end_value)
{
    // MAX_DB_SCRIPT_STRING_ID is max allowed negative value for scripts (scrpts can use only more deep negative values
    // start/end reversed for negative values
    if (start_value > MAX_DB_SCRIPT_STRING_ID || end_value >= start_value)
    {
        TC_LOG_ERROR(LOG_FILTER_SQL, "Table '%s' load attempted with range (%d - %d) reserved by ORIGIN, strings not loaded.", table, start_value, end_value+1);
        return false;
    }

    return sObjectMgr->LoadORIGINStrings(table, start_value, end_value);
}

...
template<class T> T* ObjectAccessor::GetObjectInWorld(uint32 mapid, float x, float y, uint64 guid, T* /*fake*/)
{
    ...
    CellCoord q = ORIGIN::ComputeCellCoord(obj->GetPositionX(), obj->GetPositionY());
    if (!q.IsCoordValid())
    {
...
{
    ORIGIN_READ_GUARD(HashMapHolder<Player>::LockType, *HashMapHolder<Player>::GetLock());
...
{
    ORIGIN_READ_GUARD(ACE_RW_Thread_Mutex, i_corpseLock);
...
void ObjectAccessor::RemoveCorpse(Corpse* corpse)
{
    ...
    // Critical section
    {
        ORIGIN_WRITE_GUARD(ACE_RW_Thread_Mutex, i_corpseLock);
        ...
        CellCoord cellCoord = ORIGIN::ComputeCellCoord(corpse->GetPositionX(), corpse->GetPositionY());

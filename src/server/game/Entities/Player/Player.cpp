        ...
        for (GroupReference* itr = _group->GetFirstMember(); itr != NULL; itr = itr->next())
            if (Player* member = itr->GetSource())
                if (member->IsAlive() && member->IsAtGroupRewardDistance(_victim))
                {
                    ...
                    // 2.4. _maxNotGrayMember - maximum level of alive group member within reward distance,
                    //      for whom victim is not gray;
                    uint32 grayLevel = ORIGIN::XP::GetGrayLevel(lvl);
...
inline void KillRewarder::_InitXP(Player* player)
{
    ...
    if (_isBattleGround || (!_isPvP && !_killer->GetVehicle()))
        _xp = ORIGIN::XP::Gain(player, _victim);
}
    ...
    if (_maxLevel)
    {
        ...
        if (!_isBattleGround || _xp)
        {
            ...
            if (!_isBattleGround)
            {
                ...
                _groupRate = ORIGIN::XP::xp_in_group_rate(_count, isRaid);
            }
...
void Player::Regenerate(Powers power)
{
    ...
    switch (power)
    {
        case POWER_MANA:
        {
            ...
            if (recentCast) // ORIGIN Updates Mana in intervals of 2s, which is correct
            ...
            if (CanJoinConstantChannelInZone(channel, current_zone))
            {
                if (!(channel->flags & CHANNEL_DBC_FLAG_GLOBAL))
                {
                    ...
                    if (channel->flags & CHANNEL_DBC_FLAG_CITY_ONLY)
                        currentNameExt = sObjectMgr->GetORIGINStringForDBCLocale(LANG_CHANNEL_CITY);
                    else
...
void Player::UpdateCombatSkills(Unit* victim, WeaponAttackType attType, bool defence)
{
    ...
    uint8 greylevel = ORIGIN::XP::GetGrayLevel(plevel);
...
void Player::SendActionButtons(uint32 state) const
{
    ...
    /*
        state can be 0, 1, 2
        0 - Looks to be sent when initial action buttons get sent, however on ORIGIN we use 1 since 0 had some difficulties
        1 - Used in any SMSG_ACTION_BUTTONS packet with button data on ORIGIN. Only used after spec swaps on retail.
...
void Player::SendMessageToSetInRange(WorldPacket* data, float dist, bool self, bool own_team_only)
{
    ...
    ORIGIN::MessageDistDeliverer notifier(this, data, dist, own_team_only);
    VisitNearbyWorldObject(dist, notifier);
}
...
int32 Player::CalculateReputationGain(ReputationSource source, uint32 creatureOrQuestLevel, int32 rep, int32 faction, bool noQuestBonus)
{
    ...
    if (rate != 1.0f && creatureOrQuestLevel <= ORIGIN::XP::GetGrayLevel(getLevel()))
        percent *= rate;
...
bool Player::RewardHonor(Unit* victim, uint32 groupsize, int32 honor, bool pvptoken)
{
    ...
    if (honor_f <= 0)
    {
        ...
        if (Player* plrVictim = victim->ToPlayer())
        {
            ...
            uint8 k_grey = ORIGIN::XP::GetGrayLevel(k_level);
            ...
            honor_f = ceil(ORIGIN::Honor::hk_honor_at_level_f(k_level) * (v_level - k_grey) / (k_level - k_grey));
...
// not used in ORIGIN, but used in scripting code
...
//not used in ORIGINd, function for external script library
...
void Player::_LoadInventory(PreparedQueryResult result, uint32 timeDiff)
{
    ...
    if (result)
    {
        ...
        {
            std::string subject = GetSession()->GetORIGINString(LANG_NOT_EQUIPPED_ITEM);
...
bool Player::Satisfy(AccessRequirement const* ar, uint32 target_map, bool report)
{
    if (!IsGameMaster() && ar)
    {
        ...
        {
            GetSession()->SendAreaTriggerMessage("%s", GetSession()->GetORIGINString(LANG_INSTANCE_CLOSED));
            return false;
        }
        ...
        if (LevelMin || LevelMax || missingItem || missingQuest || missingAchievement)
        {
            if (report)
            {
                if (missingQuest && !ar->questFailedText.empty())
                    ...
                    GetSession()->SendAreaTriggerMessage(GetSession()->GetORIGINString(LANG_LEVEL_MINREQUIRED_AND_ITEM), LevelMin, sObjectMgr->GetItemTemplate(missingItem)->Name1.c_str());
                else if (LevelMin)
                    GetSession()->SendAreaTriggerMessage(GetSession()->GetORIGINString(LANG_LEVEL_MINREQUIRED), LevelMin);
            }
            return false;
        }
    }
...
void Player::UpdateVisibilityForPlayer()
{
    ...
    ORIGIN::VisibleNotifier notifier(*this);
...
{
    uint8 v_level = victim->getLevel();
    uint8 k_grey  = ORIGIN::XP::GetGrayLevel(getLevel());

        ...
        case GO_READY:
        {
            ...
            if (isSpawned())
            {
                ...
                if (goInfo->type == GAMEOBJECT_TYPE_TRAP)
                {
                    ...
                    {
                        ORIGIN::AnyUnfriendlyNoTotemUnitInObjectRangeCheck checker(this, owner, radius);
                        ORIGIN::UnitSearcher<ORIGIN::AnyUnfriendlyNoTotemUnitInObjectRangeCheck> searcher(this, ok, checker);
                    ...
                    else                                        // environmental trap
                    {
                        ...
                        // affect only players
                        Player* player = NULL;
                        ORIGIN::AnyPlayerInObjectRangeCheck checker(this, radius);
                        ORIGIN::PlayerSearcher<ORIGIN::AnyPlayerInObjectRangeCheck> searcher(this, player, checker);
...
void GameObject::TriggeringLinkedGameObject(uint32 trapEntry, Unit* target)
{
    ...
    GameObject* trapGO = NULL;
    {
        ...
        ORIGIN::NearestGameObjectEntryInObjectRangeCheck go_check(*target, trapEntry, range);
        ORIGIN::GameObjectLastSearcher<ORIGIN::NearestGameObjectEntryInObjectRangeCheck> checker(this, trapGO, go_check);

        TypeContainerVisitor<ORIGIN::GameObjectLastSearcher<ORIGIN::NearestGameObjectEntryInObjectRangeCheck>, GridTypeMapContainer > object_checker(checker);
...
GameObject* GameObject::LookupFishingHoleAround(float range)
{
    ...
    CellCoord p(ORIGIN::ComputeCellCoord(GetPositionX(), GetPositionY()));
    Cell cell(p);
    ORIGIN::NearestGameObjectFishingHole u_check(*this, range);
    ORIGIN::GameObjectSearcher<ORIGIN::NearestGameObjectFishingHole> checker(this, ok, u_check);

    TypeContainerVisitor<ORIGIN::GameObjectSearcher<ORIGIN::NearestGameObjectFishingHole>, GridTypeMapContainer > grid_object_checker(checker);
            ...
            if (GetUniqueUseCount() == info->summoningRitual.reqParticipants)
            {
                ...
                if (info->summoningRitual.casterTargetSpell && info->summoningRitual.casterTargetSpell != 1) // No idea why this field is a bool in some cases
                    for (uint32 i = 0; i < info->summoningRitual.casterTargetSpellTargets; i++)
                        // m_unique_users can contain only player GUIDs
                        if (Player* target = ObjectAccessor::GetPlayer(*this, ORIGIN::Containers::SelectRandomContainerElement(m_unique_users)))

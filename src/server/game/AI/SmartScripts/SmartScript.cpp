...
class ORIGINStringTextBuilder
{
    public:
        ORIGINStringTextBuilder(WorldObject* obj, ChatMsg msgtype, int32 id, uint32 language, uint64 targetGUID)
        ...
        {
            std::string text = sObjectMgr->GetORIGINString(_textId, locale);
...
void SmartScript::ProcessAction(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, const SpellInfo* spell, GameObject* gob)
{
    ...
    switch (e.GetActionType())
    {
        ...
        case SMART_ACTION_FLEE_FOR_ASSIST:
        {
            ...
            {
                ORIGINStringTextBuilder builder(me, CHAT_MSG_MONSTER_EMOTE, LANG_FLEE, LANG_UNIVERSAL, 0);
        ...
        case SMART_ACTION_CALL_FOR_HELP:
        {
            if (me)
            {
                ...
                {
                    ORIGINStringTextBuilder builder(me, CHAT_MSG_MONSTER_EMOTE, LANG_CALL_FOR_HELP, LANG_UNIVERSAL, 0);
                    sCreatureTextMgr->SendChatPacket(me, builder, CHAT_MSG_MONSTER_EMOTE);
                }
...
ObjectList* SmartScript::GetWorldObjectsInDist(float dist)
{
    ...
    if (obj)
    {
        ORIGIN::AllWorldObjectsInRange u_check(obj, dist);
        ORIGIN::WorldObjectListSearcher<ORIGIN::AllWorldObjectsInRange> searcher(obj, *targets, u_check);
...
Unit* SmartScript::DoSelectLowestHpFriendly(float range, uint32 MinHPDiff)
{
    ...
    CellCoord p(ORIGIN::ComputeCellCoord(me->GetPositionX(), me->GetPositionY()));
    Cell cell(p);
    cell.SetNoCreate();

    Unit* unit = NULL;

    ORIGIN::MostHPMissingInRange u_check(me, range, MinHPDiff);
    ORIGIN::UnitLastSearcher<ORIGIN::MostHPMissingInRange> searcher(me, unit, u_check);

    TypeContainerVisitor<ORIGIN::UnitLastSearcher<ORIGIN::MostHPMissingInRange>, GridTypeMapContainer >  grid_unit_searcher(searcher);
...
void SmartScript::DoFindFriendlyCC(std::list<Creature*>& _list, float range)
{
    ...
    ORIGIN::FriendlyCCedInRange u_check(me, range);
    ORIGIN::CreatureListSearcher<ORIGIN::FriendlyCCedInRange> searcher(me, _list, u_check);

    TypeContainerVisitor<ORIGIN::CreatureListSearcher<ORIGIN::FriendlyCCedInRange>, GridTypeMapContainer >  grid_creature_searcher(searcher);
...
void SmartScript::DoFindFriendlyMissingBuff(std::list<Creature*>& list, float range, uint32 spellid)
{
    ...
    ORIGIN::FriendlyMissingBuffInRange u_check(me, range, spellid);
    ORIGIN::CreatureListSearcher<ORIGIN::FriendlyMissingBuffInRange> searcher(me, list, u_check);

    TypeContainerVisitor<ORIGIN::CreatureListSearcher<ORIGIN::FriendlyMissingBuffInRange>, GridTypeMapContainer >  grid_creature_searcher(searcher);
...
Unit* SmartScript::DoFindClosestFriendlyInRange(float range)
{
    ...
    Unit* unit = NULL;
    ORIGIN::AnyFriendlyUnitInObjectRangeCheck u_check(me, me, range);
    ORIGIN::UnitLastSearcher<ORIGIN::AnyFriendlyUnitInObjectRangeCheck> searcher(me, unit, u_check);
